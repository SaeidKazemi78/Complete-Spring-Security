...
export class DriverComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        for (const key of x) {
            ...
            if (value.length > 1) {
                    if (value[0] === 'active') {
                        this.driver[value[0]] = Boolean(value[1]);
                    } else {
                    this.driver[value[0]] = value[1];
                }
            }
        }
    }
    ...
    clear() {
        ...
        this.router.navigate(['car/' + this.carId + '/driver'], {
    ...
    search() {
        ...
        if (this.driver.birthCertificateNumber) {
            this.currentSearch += 'birthCertificateNumber$' + this.driver.birthCertificateNumber + '&';
        }
        if (this.driver.active) {
            this.currentSearch += 'active;' + this.driver.active + '&';
        }
    ...
    setBreadCrumb() {
        ...
        this.translateService.get('niopdcgatewayApp.driver.home.carTitle').subscribe(title => {
            this.breadcrumbItems.push({label: title + ` (${this.car.title})`, routerLink: ['/car']});
        });
        this.translateService.get('niopdcgatewayApp.driver.home.title').subscribe(title => {
